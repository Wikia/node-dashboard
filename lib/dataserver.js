...
DataServer.prototype.getSource = function( name ) {
	...
	source.on('update',function() {
		self.onSourceUpdate.apply(self,arguments);
	});
	provider.addSubscriber(source,config.data);
...
DataServer.prototype.onClientDisconnect = function( socket, data ) {
	var socketSubscriptions = socket._subscribed = socket._subscribed || {},
		sourceName, source, i;
	for (sourceName in socketSubscriptions) {
		delete socketSubscriptions[sourceName];
...
DataServer.prototype.onSourceUpdate = function( source ) {
	if ( !source.subscribed ) return;
	var sockets = source.subscribed,
		i;
	for (i=0;i<sockets.length;i++) {
		this.pushData(source,sockets[i]);
	}
};
